      ...
      >
        ...
        >
          ...
          <div className="flex justify-center">
            <button 
              className="mt-6 px-4 py-2 w-fit mx-auto cursor-pointer sm:text-sm text-black bg-white h-12 border border-transparent text-xs flex justify-center items-center rounded-2xl hover:shadow-lg transition duration-200 shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]"
  ...
  return (
    ...
    >
      <ul
        ...
        style={{
          transform: `translateX(-${current * (250 / slides.length)}%)`,
          left: '30%',
        }}
      >
      ...
      <div className="absolute flex justify-center w-full top-[calc(100%+1rem)] space-x-2">
        {slides.map((_, index) => (
          <button
            key={index}
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              current === index 
                ? "bg-custom-color-2 scale-125" 
